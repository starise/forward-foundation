...
function forward_theme_activation_options_init() {
  register_setting(
    'forward_activation_options',
    'forward_theme_activation_options'
  );
}
add_action('admin_init', 'forward_theme_activation_options_init');

function forward_activation_options_page_capability($capability) {
  return 'edit_theme_options';
}
add_filter('option_page_capability_forward_activation_options', 'forward_activation_options_page_capability');

function forward_theme_activation_options_add_page() {
  $forward_activation_options = forward_get_theme_activation_options();
...
add_action('admin_menu', 'forward_theme_activation_options_add_page', 50);

function forward_get_theme_activation_options() {
  return get_option('forward_theme_activation_options');
}
...
function forward_theme_activation_action() {
  if (!($forward_theme_activation_options = forward_get_theme_activation_options())) {
    return;
  }
  ...
  if ($forward_theme_activation_options['change_permalink_structure'] === 'true') {
    $forward_theme_activation_options['change_permalink_structure'] = false;
  ...
  if ($forward_theme_activation_options['create_navigation_menus'] === 'true') {
    $forward_theme_activation_options['create_navigation_menus'] = false;
    ...
    if ($forward_nav_theme_mod) {
      set_theme_mod('nav_menu_locations', $forward_nav_theme_mod);
    }
  }
  ...
  update_option('forward_theme_activation_options', $forward_theme_activation_options);
}
add_action('admin_init','forward_theme_activation_action');

function forward_deactivation() {
  delete_option('forward_theme_activation_options');
}
add_action('switch_theme', 'forward_deactivation');
