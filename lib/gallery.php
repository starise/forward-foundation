...
function forward_gallery($attr) {
  ...
  foreach ($attachments as $id => $attachment) {
    ...
    $output .= ($i % $columns == 0) ? '<div class="row gallery">': '';
    $output .= '<div class="' . $grid .'">' . $image;

    if (trim($attachment->post_excerpt)) {
      $output .= '<div class="caption hide">' . wptexturize($attachment->post_excerpt) . '</div>';
    }
...
if (current_theme_supports('foundation-gallery')) {
  remove_shortcode('gallery');
  add_shortcode('gallery', 'forward_gallery');
 ...
 */
function forward_attachment_link_class($html) {
  $postid = get_the_ID();
  $html = str_replace('<a', '<a class="th"', $html);
  return $html;
}
add_filter('wp_get_attachment_link', 'forward_attachment_link_class', 10, 1);
